...
func CreatePost(postID, communityID int64) error {
	...
	// 更新：把帖子id加到社区的set
	cKey := getRedisKey(KeyCommunitySetP + strconv.Itoa(int(communityID)))
	//把帖子id加到社区的set中
...
func VoteForPost(userID, postID string, value float64) error {
	...
	// 2. 更新贴子的分数
	// 先查当前用户给当前帖子的投票记录
	ov := client.ZScore(getRedisKey(KeyPostVotedZSetP+postID), userID).Val()
	...
	// 3. 记录用户为该贴子投票的数据
	if value == 0 {
		pipeline.ZRem(getRedisKey(KeyPostVotedZSetP+postID), userID)
	} else {
		pipeline.ZAdd(getRedisKey(KeyPostVotedZSetP+postID), redis.Z{
